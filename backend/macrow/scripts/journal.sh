#!/usr/bin/env bash
# Smoke: /users/$USER_ID/journal — log food, log recipe, GET day, PATCH item, bulk-delete,
...
cleanup() {
    if [ "${#ITEM_IDS[@]}" -gt 0 ]; then
        ...
        joined="[${joined%,}]"
        curl -sS -X POST "$BASE_URL/users/$USER_ID/journal/days/$TODAY/items:bulk-delete" \
...
section "POST /users/$USER_ID/journal/days/$TODAY/meals/breakfast/items (food-backed)"
req POST "/users/$USER_ID/journal/days/$TODAY/meals/breakfast/items" \
...
section "POST /users/$USER_ID/journal/days/$TODAY/meals/dinner/recipes (recipe-backed, embedded macros)"
req POST "/users/$USER_ID/journal/days/$TODAY/meals/dinner/recipes" \
...
section "GET /users/$USER_ID/journal/days/$TODAY (full embed)"
req GET "/users/$USER_ID/journal/days/$TODAY"
expect_status 200
echo "$BODY" | jq '
  .meals[]
...
section "PATCH /users/$USER_ID/journal/days/$TODAY/items/${ITEM_IDS[0]} (food, change quantity)"
req PATCH "/users/$USER_ID/journal/days/$TODAY/items/${ITEM_IDS[0]}" '{"quantity":50}'
expect_status 200
...
section "GET /users/$USER_ID/journal/days/not-a-date → 400 INVALID_DATE"
req GET "/users/$USER_ID/journal/days/not-a-date"
expect_status 400
echo "$BODY" | jq .
[ "$(echo "$BODY" | jq -r .error.code)" = "INVALID_DATE" ] || fail "wrong error code"

section "POST /users/$USER_ID/journal/days/$TODAY/meals/brunch/items → 400 INVALID_MEAL_KIND"
req POST "/users/$USER_ID/journal/days/$TODAY/meals/brunch/items" \
  "{\"barcode\":\"$BARCODE\",\"quantity\":30}"
expect_status 400
